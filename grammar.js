...
module.exports = grammar({
  ...
  rules: {
    ...
    defcfg: ($) =>
      seq("(", "defcfg", repeat(choice($.assignment, $._expression)), ")"),
    defsrc: ($) => seq("(", "defsrc", repeat($._expression), ")"),
    defalias: ($) =>
      seq("(", "defalias", repeat(choice($.assignment, $._expression)), ")"),
    deflayer: ($) =>
      seq("(", "deflayer", $.identifier, repeat($._expression), ")"),

    _expression: ($) => choice($.function_call, $.string, $.boolean, $._value),

    function_call: ($) =>
      seq("(", $.identifier, repeat(seq(/[ \t]/, $._expression)), ")"),
